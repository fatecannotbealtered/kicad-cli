...
function securityPolicy(root) {
  // SECURITY.md states which minor gets security fixes. It drifted to 0.1.0
  // while every other location moved, because it was the one version string
  // nobody had registered here -- which is exactly the failure this registry
  // exists to prevent. Both language variants share the row shape.
  const files = ["SECURITY.md", "SECURITY_zh.md"]
    .map((name) => path.join(root, name))
    .filter((f) => fs.existsSync(f));
  if (files.length === 0) return null;
  const ROW = /(\|\s*(?:latest|最新)\s*`)([0-9]+\.[0-9]+\.[0-9]+)(`)/;
  return {
    label: "SECURITY.md supported-versions table",
    read() {
      const out = [];
      for (const f of files) {
        const m = readRaw(f).match(ROW);
        if (m) out.push({ where: path.basename(f), value: m[2] });
      }
      return out;
    },
    write(version) {
      let n = 0;
      for (const f of files) {
        const text = readRaw(f);
        n += writeTextPreserving(f, text, text.replace(ROW, `$1${version}$3`));
      }
      return n;
    },
  };
