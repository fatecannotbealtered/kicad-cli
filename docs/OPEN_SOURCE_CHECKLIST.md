...
Run through this gate **before the first public push** of `kicad-cli`. It is a security and quality checkpoint, not documentation — every box must be ticked (or consciously waived with a note) before the repo goes public. Once public, secrets in history cannot be un-leaked.
...
- [x] N/A — this tool has no credentials, no config file and no `~/.kicad-cli/` directory. `SECURITY.md` states the absence explicitly, so a future version that gains one has to revisit both.
...
- [ ] Install blocks are copy-paste-runnable and use the real published `@fateforge/kicad-cli` / `fatecannotbealtered/kicad-cli`.

## Governance

- [ ] `SECURITY.md` is present with a working disclosure channel (`security@fatecannotbealtered.github.io`) and a supported-versions table.
...
- [ ] If `kicad-cli` wraps a third-party product (KiCad), `NOTICE.md` carries the trademark / non-affiliation notice and `docs/COMPATIBILITY.md` lists the verified backend version matrix.
...
- [ ] Functional Contract Coverage is 100%: every public behavior documented in README, Skill, `reference`, `--help`, `context`, `doctor`, or `changelog` has automated command-level tests.
...
- [ ] `skills/kicad-cli/SKILL.md` is present; frontmatter includes `version`, `license: MIT`, `user-invocable: true`, and `metadata.requires.min_version` matching the CLI version.
...
- [ ] `skills/kicad-cli/test-prompts.json` is present, valid JSON, and covers fresh-agent read, write safety or read-only boundary, permission boundary, `_untrusted` handling, and the absence of self-update.
- [x] N/A — there is no `update` command. SKILL-SPEC §6.7 applies only to tools that have one; `SKILL.md` instead points at the two install lines, and `tests/test_skill.py` fails if the Skill ever claims a self-update that does not exist.
- [ ] `kicad-cli reference`, `kicad-cli context`, and `kicad-cli doctor` run and emit valid JSON envelopes — an agent can self-onboard from a clean checkout.
- [ ] `kicad-cli reference` exposes `release_readiness`, and `kicad-cli doctor` reports the matching check.
