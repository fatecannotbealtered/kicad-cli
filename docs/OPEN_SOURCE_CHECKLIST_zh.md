...
在 `kicad-cli` **首次公开推送之前**逐项走查。这是一道安全与质量关卡，不是文档 —— 仓库公开前每一项都必须勾选（或明确写明理由后豁免）。一旦公开，历史中泄露的密钥就无法收回。
...
- [x] 不适用 —— 本工具没有凭据、没有配置文件，也没有 `~/.kicad-cli/` 目录。`SECURITY_zh.md` 把这个「没有」明写出来，将来哪个版本引入了凭据，两处都要重审。
...
- [ ] 安装块可直接复制运行，使用真实已发布的 `@fateforge/kicad-cli` / `fatecannotbealtered/kicad-cli`。

## 治理

- [ ] `SECURITY.md` 存在，含可用的披露渠道（`security@fatecannotbealtered.github.io`）和受支持版本表。
- [ ] `CONTRIBUTING.md` 存在（环境搭建、分支/提交、测试、PR 流程）。
- [ ] 若项目接受外部贡献，`CODE_OF_CONDUCT.md` 存在（Contributor Covenant）。
- [ ] 若 `kicad-cli` 包装第三方产品（KiCad），`NOTICE.md` 载明商标 / 非隶属声明，且 `docs/COMPATIBILITY.md` 列出已验证的后端版本矩阵。

## 构建 / CI

- [ ] 待推送的提交上 CI（`.github/workflows/ci.yml`）为**绿色**。
- [ ] CI **强制**执行 lint 和测试 —— lint 失败或测试失败会阻断合并（不仅仅是提示性的）。
- [ ] 功能契约覆盖率为 100%：README、Skill、`reference`、`--help`、`context`、`doctor`、`changelog` 中记录的每个公开行为，都有自动化命令级测试。
...
- [ ] `skills/kicad-cli/SKILL.md` 存在；frontmatter 包含 `version`、`license: MIT`、`user-invocable: true`，且 `metadata.requires.min_version` 匹配 CLI 版本。
- [ ] `SKILL.md` 包含 `When to use`、`Do not use`、`First Step`、Agent 默认规则、JSON contract、写操作配方或明确只读边界、`STOP CHECKPOINT`、错误决策树、安全边界、自更新和评估场景。
- [ ] `skills/kicad-cli/test-prompts.json` 存在、JSON 合法，并覆盖 fresh-agent read、写操作安全或只读边界、权限边界、`_untrusted` 处理和自更新。
- [x] 不适用 —— 没有 `update` 命令。SKILL-SPEC §6.7 只对有自更新的工具生效；`SKILL.md` 改为指向那两行安装命令，而 `tests/test_skill.py` 会在 Skill 声称任何不存在的自更新时失败。
- [ ] `kicad-cli reference`、`kicad-cli context`、`kicad-cli doctor` 可运行并输出合法的 JSON 信封 —— 代理能从干净的检出自助上手。
- [ ] `kicad-cli reference` 暴露 `release_readiness`，`kicad-cli doctor` 报告匹配的检查项。
