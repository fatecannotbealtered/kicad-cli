...
- **Coverage.** The guard does not trust that a test exists. It reads a dispatch
  ...
  it name the command that went unexercised.
...
10.0.6. There is no second machine behind it and no CI run, because CI has no
KiCad. The mock tests run everywhere and cover the failure paths, but they
cannot tell you that KiCad 10.0.7 still writes the file format the same way.
