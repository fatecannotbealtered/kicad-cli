...
| Board writes | `board route`, `board pour`, `board netclass`, `board rewidth`, `board widen`, `board stitch`, `board move`, `board place` | Routing (grid by default, or Freerouting via `--engine freerouting` when the user has it installed), ground pours, netclass targets, trace widening, stitching, movement and automatic placement behind a confirmation gate. Verification and rollback vary by operation and mode; they are not a uniform safety guarantee. |
