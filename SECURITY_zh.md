...
- **已有布线。** `board route --mode full` 会先清掉每一段走线再布。它在 Skill 里有专门的检查点；其他写操作也可能损坏设计数据或覆盖输出文件。它现在会在动手前先取一次 DRC error 基线，写完后 error 数上升就整盘回滚；裁判不可用时在写之前就拒绝，而不是提交一个未经验证的结果。这道判据是下限不是保证：error 数没上升的板子，仍可能在 DRC 量不到的地方更差。
