...
影响范围包括本地设计文件和生成的输出。当前已有确认入口，但认证、过期、防重放与完整目标绑定尚未完成。此开发候选版本为 **unpublishable**；见[开发状态](docs/DEVELOPMENT_STATUS.md)。合并局部修复不等于批准发布或无人值守生产写入。

## 凭证处理

**不需要服务账号凭证。** 工具操作本地设计文件和本机 KiCad。确认令牌是操作控制，不是账号凭证。设计内容和本地路径仍可能敏感；没有账号凭证不等于没有信息泄漏风险。

`KICAD_CLI_ROOT`、`KICAD_CLI_PYTHON` 和 `KICAD_CLI_OFFICIAL` 是覆盖 KiCad 定位方式的文件路径，不是认证密钥。
