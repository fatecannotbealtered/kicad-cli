...
8. **Functional Contract Coverage = 100% before release**: every public README / Skill / reference / help / context / doctor / changelog behavior has command-level tests.
...
- Tool name: `kicad-cli` — shares the name of KiCad's own binary on purpose, and never forwards to it. Everything here is implemented against KiCad's libraries, file formats and IPC API directly, so a command that exists is a command that works.
