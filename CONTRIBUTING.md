# Contributing to kicad-cli

*English | [中文](CONTRIBUTING_zh.md)*

Thanks for improving **kicad-cli** — AI-native KiCad PCB CLI - board audit, schematic parity, grid routing, zone stitching, ampacity-driven trace width, and DRC-verified writes. This document covers building, testing, and submitting changes.
...
```bash
# Clone
git clone https://github.com/fatecannotbealtered/kicad-cli.git
cd kicad-cli

# --- Go variant ---
go mod download
make build                      # or: go build -o bin/kicad-cli ./src/kicad_cli
go test -race ./...
./bin/kicad-cli --help
...
Release standard: **Functional Contract Coverage = 100%**. Every public behavior documented in `README`, `SKILL`, reference pages, `kicad-cli reference`, `--help`, `context`, `doctor`, `changelog`, or `update` must have automated command-level tests.
...
Keep `kicad-cli reference` `release_readiness` and `kicad-cli doctor`'s `release_readiness` check honest when test evidence changes.
...
5. **SKILL** — add a `skills/kicad-cli/reference/<domain>.md` page and link it from `skills/kicad-cli/SKILL.md` (keep SKILL.md a short progressive-disclosure index).
6. **Docs** — update `README.md` / `README_zh.md` command lists and add a line under `## [Unreleased]` in `CHANGELOG.md`.

`reference` walks the command tree automatically, so new commands appear in `kicad-cli reference` without extra wiring.
